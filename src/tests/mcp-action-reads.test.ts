...
const config: HarnessConfig = {
  project: { name: 'test', description: 'test', docsPath: './docs' }, provider: 'claude-code', database: { type: 'sqlite' },
  storage: { dir: '.harness', tasks: { adapter: 'local' }, sections: { toolsUsed: true, filesModified: true, result: true, blockers: true, nextSteps: false }, markdownFallback: { enabled: false, path: join(TMP, 'current.md') }, scope: 'local', projectId: 'mcp-reads-test', sqlitePath: join(TMP, 'harness.db') },
  health: { scriptPath: './health.sh', required: false }, tools: { mcp: { enabled: false, port: 3456 }, scripts: { enabled: false, outputDir: '.harness/scripts' } },
}
...
describe('compact action MCP reads', () => {
  ...
  test('actions.list is paginated metadata while actions.get stays full history', async () => {
    ...
    const filtered = text(await dispatch('actions.list', { taskId, agent: 'lead' }, db, TMP, TMP, config))
    assert.equal((filtered.items as Array<Record<string, unknown>>)[0].id, leadId)

    const legacy = text(await dispatch('actions.get', { taskId }, db, TMP, TMP, config)) as unknown as Array<Record<string, unknown>>
    assert.equal(legacy.length, 3)
    assert.equal(((legacy[0].sections as Array<Record<string, unknown>>)[0]).content, 'A long plan that must not appear in compact indexes.')
  })

  test('action and section indexes omit contents, while ranged section reads report what remains', async () => {
    const action = text(await dispatch('actions.get_by_id', { actionId: leadId }, db, TMP, TMP, config))
    const sections = action.sections as Array<Record<string, unknown>>
    assert.equal(sections.length, 2)
    assert.equal('content' in sections[0], false)

    const listed = text(await dispatch('actions.sections.list', { actionId: leadId, types: ['result'] }, db, TMP, TMP, config))
    const section = (listed.items as Array<Record<string, unknown>>)[0]
    assert.equal(section.type, 'result')
    const content = text(await dispatch('actions.sections.get', { sectionId: section.id, length: 6 }, db, TMP, TMP, config))
  ...
  test('handoffs are structured, bounded, recipient-directed, and never fall back to history', async () => {
    const missing = text(await dispatch('actions.handoff.get', { taskId, recipient: 'builder' }, db, TMP, TMP, config))
    assert.deepEqual(missing, { found: false, reason: 'HANDOFF_NOT_FOUND' })

    await dispatch('actions.handoff.write', { actionId: leadId, recipient: 'builder', goal: 'Implement compact reads', completed: ['Mapped data layer'], decisions: ['Preserve actions.get'], files: ['src/core/mcp-server.ts'], verification: ['pnpm test'], blockers: [], nextStep: 'Implement tests' }, db, TMP, TMP, config)
    ...
    await assert.rejects(() => dispatch('actions.write', { actionId: builderId, sectionType: 'handoff', content: 'free text' }, db, TMP, TMP, config), /actions\.handoff\.write/)
    await assert.rejects(() => dispatch('actions.handoff.write', { actionId: leadId, recipient: 'builder', goal: 'x'.repeat(12_001), completed: [], decisions: [], files: [], verification: [], blockers: [], nextStep: 'next' }, db, TMP, TMP, config), /12000 UTF-8 bytes/)
  })
})
