...
- **harness skills** — checks every file in each bundled skill tree, including `resources/`, against the bundled source. Reports `[!]` if any manifest or resource is missing or outdated.
...
└── .cursor/
    ...
    ├── permissions.json          ← allows this harness MCP's tools without per-call prompts
    └── agents/
